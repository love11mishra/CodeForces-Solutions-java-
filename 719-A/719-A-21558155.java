   ...
   /* public static long lcm(long a, long b){
	...
	/*static void ArrayPrint(int[]a){
	  ...
	  public static void main(String[]args)throws IOException{
		...
		for(int i=0;i<n;i++)
			a[i]=Integer.parseInt(next());
		if(n==1&&a[0]==15)op.println("DOWN");
		else if(n==1&&a[0]==0)op.println("UP");
		else if(n==1)op.println("-1");
		else{
			if(a[n-1]-a[n-2]>0&&a[n-1]!=15)op.println("UP");
			else if(a[n-1]-a[n-2]>0&&a[n-1]==15)op.println("DOWN");
			else if(a[n-1]-a[n-2]<0&&a[n-1]!=0)op.println("DOWN");
			else op.println("UP");
		}
		 op.close();
		}
